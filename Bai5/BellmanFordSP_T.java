...
public class BellmanFordSP_T {
    ...
    public BellmanFordSP_T(EdgeWeightedDigraph_T G, int s) {
        ...
        for (int v = 0; v < G.V(); v++)
            distTo[v] = Double.POSITIVE_INFINITY;  // tat ca bang vo cung
    ...
    private void relax(EdgeWeightedDigraph_T G, int v) {
        for (DirectedEdge e : G.adj(v)) {
            int w = e.from();
            if (distTo[w] > distTo[v] + e.weight()) {  // hoi disto co thoa man ko neu ngan hon thi cap nhat
                distTo[w] = distTo[v] + e.weight();
                edgeTo[w] = e;  
                if (!onQueue[w]) { // xem dinh da co trong hang doi chua
                    queue.enqueue(w); 
                    onQueue[w] = true;
                }
            }
     ...
     */
    public Iterable<DirectedEdge> negativeCycle() {// hoi co chu trinh khong
        return cycle;
    }

    // by finding a cycle in predecessor graph
    private void findNegativeCycle() {
        int V = edgeTo.length; //lay so dinh
        EdgeWeightedDigraph spt = new EdgeWeightedDigraph(V); // khoi tao 1 do thi co so dinh V
        for (int v = 0; v < V; v++)
            if (edgeTo[v] != null)  
                spt.addEdge(edgeTo[v]);  // lay cac canh edgeto vao do thi nay 

        EdgeWeightedDirectedCycle finder = new EdgeWeightedDirectedCycle(spt);  // co chu trinh ko
        cycle = finder.cycle();  // co thi tra ve chu trinh
    }
    ...
    public static void main(String[] args) {
        ...
        BellmanFordSP_T sp = new BellmanFordSP_T(G, s);
        ...
        // print shortest paths
            ...
            for (int v = 0; v < G.V(); v++) {
                if (sp.hasPathTo(v)){
                    StdOut.printf("%d to %d (%5.2f)  ", v, s, sp.distTo(v));
