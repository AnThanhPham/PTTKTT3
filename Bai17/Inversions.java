...
public class Inversions {
    ...
    private static <Key extends Comparable<Key>> long merge(Key[] a, Key[] aux, int lo, int mid, int hi) {
        ...
        // merge back to a[]
        // j la vi tri bat dau cua mang thu 2 sau ghep
        int i = lo, j = mid+1;
        for (int k = lo; k <= hi; k++) {
            if      (i > mid)                a[k] = aux[j++];
    ...
    public static void main(String[] args) throws IOException {
        // input : "input_student.csv" or "input_student2.csv"
        ...
        Student[] a = new Student[arr.size()];
        a = arr.toArray(a);
        System.out.println("So nghich the la:"+ Inversions.count(a));
    }
}
